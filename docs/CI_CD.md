...
| `.github/workflows/deploy-prod.yml`             | push to `main`, manual dispatch | Release-candidate verify → build → migrations → Vercel prod deploy → health smoke → optional live E2E when sandbox secrets exist.  |
...
| `FORGE_E2E_CLERK_SESSION`      | prod                 | Optional Clerk test-session token for the post-deploy live happy-path E2E.       |
| `FORGE_E2E_DESCRIPTION`        | prod                 | Optional prompt override for the live E2E.                                       |
| `FORGE_E2E_TIMEOUT_MS`         | prod                 | Optional live E2E polling budget.                                                |
...
`deploy-prod.yml` runs release-candidate verification and build on every push to `main`. If the Vercel / database / Sentry secrets are missing, the workflow records a clear preflight warning and skips external production systems instead of failing the whole push. Manual production deploy dispatches still fail fast when required provider secrets are missing. When `FORGE_E2E_CLERK_SESSION` is configured, production deploys also run the live Playwright trigger/poll happy path against the deployed URL.

`evals-nightly.yml` skips the paid real-API sweep when `OPENAI_API_KEY` is missing. Once that secret exists, regressions fail the workflow; Slack notifications are best-effort and never mask the real eval result.
