/**
 * POST /api/agents/[id]/resume — mirrors pause; covers same shape.
 */
...
vi.mock('@forge/ntn-wrapper', () => ({ resumeSync: vi.fn() }));
...
describe('POST /api/agents/[id]/resume', () => {
  it('returns 200 on success', async () => {
    ...
    vi.mocked(db.markAgentStatus).mockResolvedValue({ id: 'a1', status: 'active' } as never);

    const { POST } = await import('@/app/api/agents/[id]/resume/route');
    const res = await POST(
      makeRequest('http://localhost/api/agents/a1/resume', { method: 'POST' }) as never,
      makeCtx({ id: 'a1' }),
    );
  ...
  it('returns 502 when ntn resumeSync fails', async () => {
    const db = await import('@forge/db');
    vi.mocked(db.prisma.generatedAgent.findUnique).mockResolvedValue({
      id: 'a1',
      workspaceId: 'ws_1',
    ...
    vi.mocked(ntn.resumeSync).mockRejectedValue(new Error('boom'));
    const { POST } = await import('@/app/api/agents/[id]/resume/route');
    const res = await POST(
      makeRequest('http://localhost/api/agents/a1/resume', { method: 'POST' }) as never,
      makeCtx({ id: 'a1' }),
